...
class DescriptionActivity : AppCompatActivity() {
    ...
    lateinit var btnAaaToFav : Button
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnAaaToFav = findViewById(R.id.btnAddToFav)
            ...
            val jsonRequest = object: JsonObjectRequest(Request.Method.POST,url,jsonParams, Response.Listener {
                try {
                    ...
                    if (success) {
                        ...
                        if(isFav){
                            btnAaaToFav.text = "Remove from favourites"
                            val favColor = ContextCompat.getColor(applicationContext, R.color.colorFavourite)
                            btnAaaToFav.setBackgroundColor(favColor)
                        } else{
                            btnAaaToFav.text = "Add to favourites"
                            val noFavColor = ContextCompat.getColor(applicationContext, R.color.colorPrimary)
                            btnAaaToFav.setBackgroundColor(noFavColor)
                        }
    ...
    class DBAsyncTask(val context: Context, val bookEntity: BookEntity, val mode : Int) : AsyncTask<Void,Void,Boolean>() {
        ...
        Mode 3 -> Remove thr favourite book
        */
        val db = Room.databaseBuilder(context, BookDatabase.BookDatabase::class.java, "book-db").build()
